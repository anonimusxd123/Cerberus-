...
class SettingsManager(context: Context) {
    ...
    fun set(transform: (Settings) -> Settings) { mutable.value = transform(mutable.value); prefs.edit().putBoolean("ads", mutable.value.blockAds).putBoolean("trackers", mutable.value.blockTrackers).putBoolean("boot", mutable.value.startAtBoot).putBoolean("update", mutable.value.autoUpdate).apply() }
